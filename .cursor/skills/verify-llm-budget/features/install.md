...
- **Grok hooks.** Run `verify-llm-budget home-file .grok/hooks/llm-budget.json`. JSON `hooks.PreToolUse[0].hooks[0].type` is `command`, `hooks.PreToolUse[0].hooks[0].command` points at the `grok-hook` wrapper under isolated HOME, and `hooks.PreToolUse[0].hooks[0].timeout` is `10`. The wrapper file exists, is executable, and contains `"decision":"deny"` (the no-Node fallback deny JSON) — grep it with `home-file .local/share/llm-budget/bin/grok-hook`.
