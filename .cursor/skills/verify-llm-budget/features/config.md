...
- **Invalid file, status still prints.** Overwrite isolated `config.jsonc` with `{not-json`. Run `verify-llm-budget capture config/invalid-status -- status`. Exit `0`. stdout contains `Warning: using defaults because config failed to load.` and still lists Claude Code, Codex, GitHub Copilot, Cursor Agent, and Grok CLI.
